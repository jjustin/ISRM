...
import sic.mnemonic.MnemonicD;
import sic.mnemonic.MnemonicDn;
import sic.mnemonic.MnemonicF1;
import sic.mnemonic.MnemonicF2n;
import sic.mnemonic.MnemonicF2r;
import sic.mnemonic.MnemonicF2rn;
import sic.mnemonic.MnemonicF2rr;
import sic.mnemonic.MnemonicF3;
import sic.mnemonic.MnemonicF3m;
import sic.mnemonic.MnemonicF4m;
import sic.mnemonic.MnemonicSd;
import sic.mnemonic.MnemonicSn;
import sic.node.Comment;
...
public class Parser {
	...
	public int parseNumber(int lo, int hi) throws SyntaxError {
		...
		// check range
		if (num < lo || num > hi)
			throw new SyntaxError(String.format("Number '%d' out of range [%d..%d]", num, lo, hi), lexer.row,
					lexer.col);
	...
	public Node parseInstruction() throws SyntaxError {
		...
		// skip whitespace: if EOL and without label then continue (i.e. skip empty
		// lines)
	...
	public Code parse(String input) throws SyntaxError {
	...
	void put34(String mnemonic, int opcode, String hint, String desc) {
		put(new MnemonicF3m(mnemonic, opcode, hint, desc));
		put(new MnemonicF4m("+" + mnemonic, opcode, hint, desc));
	}

	void initMnemonics() {
		this.mnemonics = new HashMap<String, Mnemonic>();
		// Directives
		put(new MnemonicD("NOBASE", Directives.NOBASE, "directive", "Unset base register."));
		put(new MnemonicD("LTORG", Directives.LTORG, "directive", "Flush literals."));
		put(new MnemonicDn("START", Directives.START, "directive", "Set code start address."));
		put(new MnemonicDn("END", Directives.END, "directive", "End code."));
		put(new MnemonicDn("BASE", Directives.BASE, "directive", "Set base register."));
		put(new MnemonicDn("EQU", Directives.EQU, "directive", "Equate symbol to expression."));
		put(new MnemonicDn("ORG", Directives.ORG, "directive", "Set location counter."));
		// Storage directives
		put(new MnemonicSn("RESB", StorageCodes.RESB, "storage\t", "Reserve bytes."));
		put(new MnemonicSn("RESW", StorageCodes.RESW, "storage\t", "Reserve words."));
		put(new MnemonicSd("BYTE", StorageCodes.BYTE, "storage\t", "Initialize bytes."));
		put(new MnemonicSd("WORD", StorageCodes.WORD, "storage\t", "Initialize words."));
