...
# NOTE:
#
# This file does not contain any code used to produce final solutions. It is only here as a failed concept
####
...
run <- function(){
  for(i in 1:3){
    ...
    setSites(i)
    sq <- generateStarting()
    maxit <- params[[problemIx]]$maxit #number of iterations
    tmax <- params[[problemIx]]$tmax # iterations for each temp
...
params <- list(
  list(retries=100, maxit=50, tmax=20, temp=10), #1
