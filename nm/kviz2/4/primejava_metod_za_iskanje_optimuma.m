function primejava_metod_za_iskanje_optimuma
format long;
check = struct();
...
%
% =============================================================================
% Primejava metod za iskanje optimuma
% =====================================================================@019203=
% 1. podnaloga
% Primerjaj tri metode za iskanje minimuma funkcije 
% $f\colon\mathbb{R}^n\to\mathbb{R}$.
% 
%  * Gradientni spust: $x_{r+1} = x_r - \eta \nabla f(x_r)$
%  * Newtonova metoda: $x_{r+1} = x_r - \eta (Hf)(x_r)^{-1}\nabla f(x_r)$
%  * Gradientni spust z momentom: $\Delta x_{r+1} = \alpha \Delta x_r -
%  \eta \nabla f(x_r), x_{r+1} = x_r + \Delta x_{r+1}$.
% 
% <br> Optimizirati poskusi 2 funkciji: kvadratno formo $f(x) = x^T Q x$ za
% $Q = [4, 1; -1, 2]$ in Rosenbrockovo funkcijo za $a=1$ in $b=100$. 
% Za zacetni priblizek vzemi $(2, -1)$.
% 
% Primerjaj stevilo korakov in cas izvajanja za $\eta = 1$ in $\alpha =
% 0.5$. Korake tudi narisi na graf nivojnic funkcije.
% =============================================================================




function x=gradientni_spust(grad, x0, eta, tol, maxiter)
    x = zeros(size(x0,1), maxiter);
    x(:, 1) = x0;
    for n=2:maxiter
        x(:, n) = x(:, n-1) - eta*(grad(x(:, n-1)));
        if (norm(x(:, n-1) - x(:,n)) < tol)
            x = x(:, 1:n-1);
            return 
        end 
    end
    warning("Gradientni spust - Reached maxiter")
end

function x=newtonova_metoda(grad, hf, x0, eta, tol, maxiter)
    x = zeros(size(x0,1), maxiter);
    x(:, 1) = x0;
    for n=2:maxiter
        x(:, n) = x(:, n-1) - eta*(inv(hf(x(:, n-1))) * grad(x(:, n-1)));
        if (norm(x(:, n-1) - x(:,n)) < tol)
            x = x(:, 1:n);
            return 
        end 
...
function x=spust_z_momentom(grad, a, x0, eta, tol, maxiter)
    x = zeros(size(x0,1), maxiter);
    x(:, 1) = x0;
    dx = zeros(size(x0,1), maxiter);
    dx(:, 1) = [0,0]';
    for n=2:maxiter
        dx(:, n) = a * dx(:, n-1) - eta*(grad(x(:, n-1)));
        x(:, n)= x(:, n-1) + dx(:, n);
        if (norm(x(:, n-1) - x(:,n)) < tol)
            x = x(:, 1:n);
            return 
...
xg = gradientni_spust(gradf, [2,1]', 0.3, 10^-3, 40);
xn = newtonova_metoda(gradf, Hf, [2,1]', 0.3, 10^-3, 40);
xm = spust_z_momentom(gradf, 0.5, [2,1]', 0.3, 10^-3, 40);
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxOTIwMywidXNlciI6NDA0Nn0:1knf2A:tTx0k5VbgI9x6_WL2wPaB3wGo3U';
        try
              check_equal('21', 21);
          catch err
