function lu_razcep_brez_pivotiranja
...
% =============================================================================
    function [L, U] = lurazcep(A)
        ...
        for k = 1:size(A)-1
            ...
            for i = k+1:size(A) % vrstica
                for j = k+1:size(A)% stoplec
...
% =============================================================================
    ...
    function y = direktnoVstavljanje(M, b)
        for i = size(M)
        end
    end


    function y = obratnoVstavljanje(M, b):
        y = 0;
        for i = 1:size(M)
        end
    end
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxODk1NCwidXNlciI6NDA0Nn0:1kdWTi:8hNoYoW_XB9R_MmpTzpW18eM6IU';
        try
              check_equal('lurazcep([2 3 8; 4 3 2; 6 7 3])', [1 0 0; 2 1 0; 3 2/3 1]);
  check_secret(lurazcep(rand(5)));
  check_secret(lurazcep(rand(10)));
  check_secret(lurazcep(rand(1)));
  check_secret(lurazcep(rand(2)));
          catch err
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxODk1NywidXNlciI6NDA0Nn0:1kdWTi:mDpdXbPBKET7s4mUU2tVP2cwZ4Y';
        try
              A = [0.4893    0.3692    0.3897
       0.3377    0.1112    0.2417
       0.9001    0.7803    0.4039];
  b = [0.1234; 0.6543; 0.1238];
  check_equal(['resi_z_lu([1 0 0;0.690169630083793 1 0;1.83956672797875 -0.704209471417423 1],'...
      '[0.4893 0.3692 0.3897;0 -0.143610627426936 -0.0272591048436542;0 0 -0.332175273706579],'... 
      '[0.1234; 0.6543; 0.1238])'], A\b);
          catch err
              check_error(['Testi sprozijo izjemo\n' err.message]);
          end
      end
