...
% =====================================================================@018492=
% 1. podnaloga
% Napisi funkcijo `x = navadna_iteracija(g, x0, tol, maxiter)`, ki izvede 
% najvec `maxiter` korakov navadne iteracije z iteracijsko funkcijo `g`
% z zacetnim priblizkom `x0`. Iteracija naj se ustavi, ko je razlika med
% zadnjima priblizkoma manjsa od `tol`. Vrne naj zaporedje priblizkov.
% 
% Primer:
% 
%     >> navadna_iteracija(@(x) sin(x.^2), 0.5, 1e-6, 100)
%     ans =
%        0.500000000000000
%        0.247403959254523
%        0.061170506395042
...
%        0.000000000000000
% 
%     >> navadna_iteracija(@(x) sin(x.^2), 0.5, 1e-6, 3)
%     ans =
%        0.500000000000000
%        0.247403959254523
%        0.061170506395042
...
% =====================================================================@018493=
% 2. podnaloga
% Preveri delovanje funkcije na resevanju enacbe $x^2 + \log x = 0$.
% Uporabi iteracijske funkcije:
% 
%   * $g_1(x) = \sqrt{-\log x}$
%   * $g_2(x) = \exp(-x^2)$
%   * $g_3(x) = (x+\exp(-x^2))/2$
%   * $g_4(x) = (2x^3+\exp(-x^2))/(1+2x^2)$
% 
% Narisi graf napak priblizkov v logaritemski skali. Tocno resitev dobis z
% `fzero`. Oglej si hitrosti za razlicne zacetne priblizke.
% =============================================================================
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxODQ5MiwidXNlciI6NDA0Nn0:1kTKrN:jpTStGld-A2pgqUdssHumLINPoo';
        try
              check_equal('length(navadna_iteracija(@(x) sin(x.^2), 0.5, 1e-6, 100))', 7);
  check_equal('length(navadna_iteracija(@(x) sin(x.^2), 0.5, 1e-6, 5))', 6);
          catch err
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxODQ5MywidXNlciI6NDA0Nn0:1kTKrN:PYpfYwTv2e60ECgFKANKJu8jzU4';
        try
              % % =====================================================================@000000=
