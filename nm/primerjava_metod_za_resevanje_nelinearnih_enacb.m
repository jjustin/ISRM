...
% =============================================================================
    ...
    function [it, n] = tangentna(f, df, x0, tol, maxiter, nicla)
        it = x0;
    ...
    function [x_curr, n] = sekantna(f, x0, x1, tol, maxiter, nicla)
        x_prev = x0;
        x_curr = x1;
        f_prev = f(x_prev);
        f_curr = f(x_curr);
    ...
    function [it, n] = fff(f, df, ddf, x0, tol, maxiter, nicla)
            it = x0;
    ...
    function [it, n] = halley(f, df, ddf, x0, tol, maxiter, nicla)
            it = x0;
...
function x = dothis()
    ...
    x0s = 1:0.1:10;
    nmetod = 4;
    tang = 1; sek = 2; ff = 3; ha = 4;
    its = zeros(length(x0s), nmetod);
    time = zeros(length(x0s), nmetod);
    neval = zeros(length(x0s), nmetod);
    
    for i = 1:length(x0s)
        num_eval = 0;
        tic
        [~, it] = tangentna(@f, @df, x0s(i), tol, maxiter, nicla);
        time(i, tang) = toc;
        its(i, tang) = it;
        neval(i, tang) = num_eval;

        num_eval = 0;
        tic
        [~, it] = sekantna(@f, x0s(i), x0s(i)*1.1, tol, maxiter, nicla);
        its(i, sek) = it;
        time(i, sek) = toc;
        neval(i, sek) = num_eval;
 
        num_eval = 0;
        tic
        [~, it] = fff(@f, @df, @ddf, x0s(i), tol, maxiter, nicla);
        its(i, ff) = it;
        time(i, ff) = toc;
        neval(i, ff) = num_eval;
        ...
        num_eval = 0;
        tic
        [~, it] = halley(@f, @df, @ddf, x0s(i), tol, maxiter, nicla);
        its(i, ha) = it;
        time(i, ha) = toc;
        neval(i, ha) = num_eval;
    end
    ...
    figure
    hold on
    plot(x0s, its(:, tang), 'or', 'displayname', 'tangentna')
    plot(x0s, its(:, sek), 'kx', 'displayname', 'sekantna')
    plot(x0s, its(:, ff), 'b+', 'displayname', '(f, f'', f'''')')
    plot(x0s, its(:, ha), 'gv', 'displayname', 'Halley')
    grid on
    xlabel('initial guess x_0')
    ylabel('number of iterations')
    legend('Location', 'NW')
    
    figure
    hold on
    plot(x0s, time(:, tang), 'or', 'displayname', 'tangentna')
    plot(x0s, time(:, sek), 'kx', 'displayname', 'sekantna')
    plot(x0s, time(:, ff), 'b+', 'displayname', '(f, f'', f'''')')
    plot(x0s, time(:, ha), 'gv', 'displayname', 'Halley')
    grid on
    % ylim([0, 0.4e-3])
    xlabel('initial guess x_0')
    ylabel('time [s]')
    ...
    figure
    hold on
    plot(x0s, neval(:, tang), 'or', 'displayname', 'tangentna')
    plot(x0s, neval(:, sek), 'kx', 'displayname', 'sekantna')
    plot(x0s, neval(:, ff), 'b+', 'displayname', '(f, f'', f'''')')
    plot(x0s, neval(:, ha), 'gv', 'displayname', 'Halley')
    grid on
    xlabel('initial guess x_0')
    ylabel('number of function evalvations')
    legend('Location', 'NW')
end
dothis()
...
function validation = validate_current_attempt_file(src)
  ...
  check_initialize(file_parts);
      ...
      if check_part()
        check.current_part.token = 'eyJwYXJ0IjoxODU2MywidXNlciI6NDA0Nn0:1kVslI:ep_-r7XjqvtV4ZM_0pSocnIfQUU';
        try
              % % =====================================================================@000000=
  ...
  token = 'Token e8ee4e1ac2e93a323737925b3400d236be58ecec';
